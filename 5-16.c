...
int strcapcmp(char *x, char *y)
{
	if (dirorder > 0)
	{
		while (!isalpha(*x) && !isdigit(*x) && !isspace(*x) && *x)
		{
			x++;
		}
		while (!isalpha(*y) && !isdigit(*y) && !isspace(*y) && *y)
		{
			y++;
		}
	}

	while ((capital > 0) ? (toupper(*x) == toupper(*y)) : (*x == *y))
	{
		if (*x == '\0')
		{
			return 0;
	...
	return (capital > 0) ? (toupper(*x) - toupper(*y)) : (*x - *y);
}
